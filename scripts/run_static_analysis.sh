#!/usr/bin/env bash
# Execute one explicitly authorized static-analysis profile and sanitize its output.
set -uo pipefail
...
if [ "$SECRET_SCAN_MODE" = 'off' ]; then
  cat "$tmp_output"
  printf '%s\n' '# Pre-Commit Review Controlled Static Analysis Secret Scan' >&2
...
PRE_COMMIT_REVIEW_SANITIZE_STREAM='controlled-static-analysis-stdout' \
