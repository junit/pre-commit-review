#!/usr/bin/env bash
# Execute one explicitly authorized analyzer manifest and sanitize its output.
set -uo pipefail
...
if [ ! -r "$STATIC_ANALYSIS_RESOLVER" ]; then
  printf '%s\n' 'orchestrate_static_analysis: trusted Rust static-analysis CLI resolver is unavailable' >&2
  exit 2
fi
...
if ! static_analysis_bin="$(resolve_static_analysis_cli "$SCRIPT_DIR")"; then
  printf '%s\n' 'orchestrate_static_analysis: trusted Rust static-analysis CLI is unavailable or invalid' >&2
  exit 2
fi

orchestrator_exit=0
"$static_analysis_bin" orchestrate "$@" >"$tmp_output" 2>"$tmp_error" \
  || orchestrator_exit=$?
...
if [ "$SECRET_SCAN_MODE" = 'off' ]; then
  cat "$tmp_output"
  printf '%s\n' '# Pre-Commit Review Static Analysis Orchestration Secret Scan' >&2
