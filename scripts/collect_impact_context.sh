...
if [ ! -r "$RESOLVER" ]; then
  emit_unavailable "$source_name" "$expected_scope" 'resolver-unavailable'
  exit 0
fi
...
if [ "$resolver_exit" -ne 0 ] || [ -z "$repository_context_bin" ]; then
  emit_unavailable "$source_name" "$expected_scope" 'binary-unavailable'
  exit 0
fi
...
if [ "$collector_exit" -ne 0 ] && [ "$collector_exit" -ne 3 ]; then
  cat "$tmp_error" >&2
  emit_unavailable "$source_name" "$expected_scope" 'collection-failed'
  exit 0
fi
