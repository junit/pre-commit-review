#!/usr/bin/env bash
# Build multi-platform release binaries for collect-diff-context-cli
set -euo pipefail
...
echo "=========================================="
echo " Building Multi-Platform Release Binaries "
echo "=========================================="

# 1. macOS ARM64 & AMD64 (Native Cargo)
if [ "$(uname -s)" = "Darwin" ]; then
  echo "[1/4] Building macOS arm64..."
...
# 3. Linux AMD64 (Docker)
echo "[3/4] Building Linux amd64 via Docker..."
docker run --rm --platform linux/amd64 \
  -v "${REPO_ROOT}:/volume" \
  -w /volume/collect-diff-context-cli \
  rust:latest cargo build --release >/dev/null

cp "${CLI_DIR}/target/release/collect-diff-context-cli" "${BIN_DIR}/collect_diff_context-linux-amd64"
