...
config.write_text(json.dumps({
    ...
    'pack_version': '2026.07.27-pcr.3', 'platform_id': 'linux-amd64',
...
subject_names = {
    'pack': 'pre-commit-review-rust-analyzer-2026.07.27-pcr.3-linux-amd64.tar.gz',
...
for platform in ['darwin-amd64', 'darwin-arm64', 'windows-amd64']:
    ...
    config.write_text(json.dumps({
        'compression': 'gzip-level-9', 'gzip_mtime': 0, 'gzip_os': 255,
        'pack_version': '2026.07.27-pcr.3', 'platform_id': platform,
        'rust_toolchain': '1.95.0', 'tar_format': 'posix-ustar'
    }, separators=(',', ':')), encoding='utf-8')
    config_sha256 = hashlib.sha256(config.read_bytes()).hexdigest()
    release['materials'] = {
    ...
    release['composition']['generator_configuration_sha256'] = config_sha256
    names = {
        'pack': f'pre-commit-review-rust-analyzer-2026.07.27-pcr.3-{platform}.tar.gz',
...
export GITHUB_REF='refs/tags/artifact-rust-analyzer-2026.07.27-pcr.3'
