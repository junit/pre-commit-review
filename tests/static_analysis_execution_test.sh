...
spec = importlib.util.spec_from_file_location('controlled_runner', module_path)
module = importlib.util.module_from_spec(spec)
sys.modules[spec.name] = module
spec.loader.exec_module(module)
object_id = 'a' * 40
stream = io.BytesIO(f'{object_id} blob 2000000\n'.encode())
try:
    module.read_batch_blob(stream, object_id, 1024)
except module.RunnerError as exc:
    assert 'exceeds the remaining snapshot byte limit' in str(exc)
else:
    raise AssertionError('oversized declared blob was accepted')
PY

python3 - "$repo_root/scripts/run_static_analysis.py" <<'PY' \
  || fail 'profile authorization was not bound to the bytes that were parsed'
import copy
import hashlib
import importlib.util
import json
...
module_path = pathlib.Path(sys.argv[1])
spec = importlib.util.spec_from_file_location('controlled_runner_profile', module_path)
module = importlib.util.module_from_spec(spec)
sys.modules[spec.name] = module
spec.loader.exec_module(module)
approved = {
    ...
    'executable': {'path': '/bin/true', 'sha256': '0' * 64},
    'arguments': [],
...
approved_bytes = json.dumps(approved, separators=(',', ':')).encode()
replacement = copy.deepcopy(approved)
replacement['name'] = 'unauthorized replacement'
replacement_bytes = json.dumps(replacement, separators=(',', ':')).encode()
...
with tempfile.TemporaryDirectory() as temporary:
    ...
    original_hasher = module.sha256_file

    def replace_after_hash(candidate):
        result = original_hasher(candidate)
        candidate.write_bytes(replacement_bytes)
        return result

    module.sha256_file = replace_after_hash
    profile, observed_hash = module.load_profile(path, expected_hash)
    assert observed_hash == expected_hash
    assert profile['name'] == 'approved profile'
PY
